//! Single-key push-to-talk via the `fn` (globe) key.
//!
//! The `fn` key is a modifier, so it never emits normal key events — only
//! `FlagsChanged`, with the `SecondaryFn` flag toggling on press/release. We
//! install a **listen-only** CGEventTap (which requires the Input Monitoring
//! permission) on a dedicated thread running its own CFRunLoop, and translate
//! fn-down / fn-up into `PttSink::start` / `stop`.
//!
